...
template <typename Dtype>
void _get_region_box(vector<Dtype> &b, Dtype* x, vector<Dtype> biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride) {
...
template <>
void _get_region_box<float>(vector<float> &b, float* x, vector<float> biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride) {
...
template <>
void _get_region_box<double>(vector<double> &b, double* x, vector<double> biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride) {
  ...
  Dtype delta_region_box(vector<Dtype> truth, Dtype* x, vector<Dtype> biases, int n, int index, int i, int j, int lw, int lh, int w, int h, Dtype* delta, float scale, int stride,IOU_LOSS iou_loss,float iou_normalizer) {
    ...
    _get_region_box(pred, x, biases, n, index, i, j,lw,lh, w, h, stride);
  ...
  void Yolov3Layer<Ftype, Btype>::LayerSetUp(
	...
	//-----------------------
	//LOG(INFO) << "w,h,"<< side_w_<<" "<< side_h_;	
	//LOG(INFO) <<"anchors,obj,class,coord,thresh"<<anchors_scale_<<" "<<object_scale_<<" "<<class_scale_<<" "<<coord_scale_<<" "<<thresh_;
	//LOG(INFO) <<"logic,focal,iou"<<use_logic_gradient_<<" "<<use_focal_loss_<<" "<<iou_loss_;
	//-----------------------
    diff_ = Blob::create<Dtype>();
    real_diff_ = Blob::create<Dtype>();
  ...
  template <typename Ftype, typename Btype>
  void Yolov3Layer<Ftype, Btype>::Reshape(
    const vector<Blob*>& bottom, const vector<Blob*>& top) {
  ...
  template <typename Ftype, typename Btype>
  void Yolov3Layer<Ftype, Btype>::Forward_cpu(
    const vector<Blob*>& bottom, const vector<Blob*>& top) {
    side_w_ = bottom[0]->width();
    side_h_ = bottom[0]->height();
	//LOG(INFO)<<"YoloV3Layer Forward_cpu";
    //LOG(INFO)<<"iou loss" << iou_loss_<<","<<GIOU;
    //LOG(INFO) << side_*anchors_scale_;
    const Dtype* label_data = bottom[1]->cpu_data<Ftype>(); //[label,x,y,w,h]
    if (diff_->width() != bottom[0]->width()) {
      diff_->ReshapeLike(*bottom[0]);
    }
    Dtype* diff = diff_->mutable_cpu_data<Ftype>();
    ...
    const Dtype* input_data = bottom[0]->cpu_data<Ftype>();
	/*const Dtype* input_data;
	  if (side_w_ == 10) {
		input_data = (Dtype*)&conv19_data[0];
		LOG(INFO) <<"CONV19_YOLO";
	  } else {
	    input_data = (Dtype*)&conv20_data[0];
		LOG(INFO) <<"CONV20_YOLO";
	  }*/
    ...
    for (int b = 0; b < bottom[0]->num(); b++) {
		...
		for (int t = 0; t < 300; ++t) {
          ...
          float top = (y - h / 2.);
          float bot = (y + h / 2.);
		  int w___ = bottom[0]->width();
		  int h___ = bottom[0]->height();
		  left*=w___; right*=w___;
		  top*=h___; bot*=h___;
		  LOG(INFO) << "Truth box" << "," << c << "," << x << "," << y << "," << w << "," << h;
		  LOG(INFO) <<"bottom w h = " << w___ << "c" << h___;
          LOG(INFO) << "Rect " << "=" <<  left << "," << top << "," << right << "," << bot;
		  }
#endif
      /*//if (b == 0) {
        char buf[100];
      ...
      for (int s = 0; s < stride; s++) {
        for (int n = 0; n < num_; n++) {
          ...
          int y2 = s / side_w_;
          int x2 = s % side_w_;
          _get_region_box(pred, swap_data, biases_, mask_[n], index, x2, y2, side_w_, side_h_, side_w_*anchors_scale_, side_h_*anchors_scale_, stride);
          for (int t = 0; t < 300; ++t) {
            vector<Dtype> truth;
            Dtype x = label_data[b * 300 * 5 + t * 5 + 1];
            Dtype y = label_data[b * 300 * 5 + t * 5 + 2];
  ...
  template <typename Ftype, typename Btype>
  void Yolov3Layer<Ftype, Btype>::Backward_cpu(const vector<Blob*>& top,
    const vector<bool>& propagate_down, const vector<Blob*>& bottom) {
	//LOG(INFO)<<"YoloV3Layer Backward_cpu";
    ...
    if (propagate_down[0]) {
      if (use_logic_gradient_) {
        const Dtype* top_data = swap_->cpu_data<Dtype>();
        Dtype* diff = diff_->mutable_cpu_data<Dtype>();
        side_w_ = bottom[0]->width();
        side_h_ = bottom[0]->height();
        int len = 4 + num_class_ + 1;
        int stride = side_w_*side_h_;
      ...
      const Dtype sign(1.);
      const Dtype alpha = sign * top[0]->cpu_diff<Dtype>()[0] / bottom[0]->num();
  ...
  INSTANTIATE_CLASS_FB(Yolov3Layer);
